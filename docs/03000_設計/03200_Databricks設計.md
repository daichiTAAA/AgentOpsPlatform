...
知識・分析基盤として既存資産の索引化/検索/分析を提供し、VS Code拡張およびAIエージェントのRAG/意思決定を支えます。

```mermaid
graph TD
    A[Databricks基盤] --> B[知識管理]
    A --> C[分析機能]
    A --> D[RAG支援]
...
| 要件分類       | 対応要件ID           | 概要                                            |
| -------------- | -------------------- | ----------------------------------------------- |
| **データ管理** | FR-009〜011          | 索引化・検索・アクセス制御                      |
| **API統合**    | FR-033〜041          | Statement Execution・Vector Search・Gateway API |
| **非機能**     | NFR-006/007/010〜023 | セキュリティ・性能・監査・データ管理            |

### ❌ 対象外

- 🚫 **VS Code拡張からの書込み操作**（FR-026参照）
